...
static void S_AL_BufferUseDefault(sfxHandle_t sfx)
{
	...
	Com_WPrintf( "WARNING: Using default sound for %s\n", knownSfx[sfx].filename);
...
static srcHandle_t streamSourceHandles[MAX_RAW_SAMPLES];
static qboolean streamPlaying[MAX_RAW_SAMPLES];
static ALuint streamSources[MAX_RAW_SAMPLES];
static ALuint streamBuffers[MAX_RAW_SAMPLES][MAX_STREAM_BUFFERS];
static int streamNumBuffers[MAX_RAW_SAMPLES];
static int streamBufIndex[MAX_RAW_SAMPLES];
...
static void S_AL_AllocateStreamChannel(int stream, int entityNum)
{
	...
	if ((stream < 0) || (stream >= MAX_RAW_SAMPLES))
		return;
